...
TEST(UInt32x4, LanesMasksAndShifts) {
	...
	for (Scalar value : values) {
		auto a = Deep::UInt32x4::s_Replicate(value);
		EXPECT_EQ(Deep::UInt32x4::s_Equals(a, a).ToBooleanBitMask(), 15u);
		...
		EXPECT_EQ((a ^ a), Deep::UInt32x4::s_Replicate(0));
		EXPECT_EQ((a & ~a), Deep::UInt32x4::s_Replicate(0));
		EXPECT_EQ((a | ~a), Deep::UInt32x4::s_Replicate(static_cast<Scalar>(~Unsigned{ 0 })));
	}
...
TEST(UInt32x4, ArithmeticAndSelect) {
	Deep::UInt32x4 a{ 12, 24, 36, 48 };
	Deep::UInt32x4 b{ 3, 4, 6, 8 };
	auto sum = a + b;
	auto difference = a - b;
	auto product = a * b;
	auto quotient = a / b;
	...
	EXPECT_EQ(Deep::UInt32x4::s_Min(a, b), b);
	EXPECT_EQ(Deep::UInt32x4::s_Max(a, b), a);
	for (Deep::uint32 mask = 0; mask < 16; ++mask) {
		Deep::Int32x4 control;
		for (size_t i = 0; i < 4; ++i)
			control[i] = (mask & (1u << i)) ? Deep::int32(0x80000000) : 1;
		auto selected = Deep::UInt32x4::s_Select(a, b, control);
		for (size_t i = 0; i < 4; ++i)
			EXPECT_EQ(selected[i], (mask & (1u << i)) ? b[i] : a[i]);
	}
}

TEST(UInt32x4, ConstructorsAndConstants) {
	constexpr auto constant = Deep::UInt32x4::Constexpr(1, 2, 3, 4);
	static_assert(constant.m_values[0] == 1 && constant.m_values[3] == 4);
	Deep::UInt32x2 low{ 1, 2 };
	Deep::UInt32x2 high{ 3, 4 };
	Deep::UInt32x4 combined{ low, high };
	Deep::UInt32x4 extended{ low };
	EXPECT_EQ(combined, (Deep::UInt32x4{ 1, 2, 3, 4 }));
	EXPECT_EQ(extended, (Deep::UInt32x4{ 1, 2, 0, 0 }));
	EXPECT_EQ(Deep::UInt32x2{ combined }, low);
	Deep::UInt32x4 native{ combined.m_internal };
	EXPECT_EQ(native, combined);
	combined[3] = Deep::uint32{ 9 };
