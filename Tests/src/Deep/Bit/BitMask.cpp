...
template<typename T>
concept AcceptsNonzeroBitMask = requires(T a) { T{ 1 }; };

template<typename T>
concept AssignsNonzeroBitMask = requires(T a) { a = 1; };

} // namespace

TEST(BitMask, LiteralZero) {
	auto check = [this]<Deep::StoragePolicy in_policy>() {
		using Mask = Deep::BitMask<4097, in_policy>;
		static_assert(Deep::c_BitMask<Mask>);
		static_assert(!AcceptsNonzeroBitMask<Mask>);
		static_assert(!AssignsNonzeroBitMask<Mask>);
