...
Vec2::Vec2(Vec3Arg in_vec) :
    x(in_vec.x), y(in_vec.y) {}

Vec2::Vec2(Vec4Arg in_vec) :
    x(in_vec.x), y(in_vec.y) {}
...
Deep_Inline bool operator!=(Vec2Arg in_a, Vec2Arg in_b) {
    return in_a.x != in_b.x || in_a.y != in_b.y;
}
Deep_Inline bool operator==(Vec2Arg a, Vec2Arg b) {
    return !(a != b);
}

Vec2& Vec2::operator+=(Vec2Arg in_other) {
    x += in_other.x;
    y += in_other.y;
    return *this;
}

Vec2 operator+(Vec2Arg in_a, Vec2Arg in_b) {
...
Vec2 operator*(Vec2Arg in_vec, float32 in_val) {
    return Vec2{ in_vec.x * in_val, in_vec.y * in_val };
}

Vec2 operator*(float32 in_val, Vec2Arg in_vec) {
