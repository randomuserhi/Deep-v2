...
float32 Vec2::m_SqrdMagnitude() const {
	return s_Dot(*this, *this);
}
float32 Vec2::m_Magnitude() const {
	return Sqrt(m_SqrdMagnitude());
}

float32 Vec2::s_Dot(Arg_Vec2 in_a, Arg_Vec2 in_b) {
...
float32 Vec2::s_Cross(Arg_Vec2 in_a, Arg_Vec2 in_b) {
	return in_a.m_values[0] * in_b.m_values[1] - in_a.m_values[1] * in_b.m_values[0];
}

Vec2 Vec2::s_Rotate(Arg_Vec2 in_a, float in_radians) {
	float32 cos = Cos(in_radians);
	float32 sin = Sin(in_radians);
	return Vec2{ in_a.x * cos - in_a.y * sin, in_a.x * sin + in_a.y * cos };
}

float32 Vec2::s_Angle(Arg_Vec2 in_a, Arg_Vec2 in_b) {
	float32 dot = Vec2::s_Dot(in_a, in_b);
	float32 cross = Vec2::s_Cross(in_a, in_b);
	return ATan2(cross, dot);
}

Vec2 Vec2::s_Lerp(Arg_Vec2 in_a, Arg_Vec2 in_b, float32 in_t) {
