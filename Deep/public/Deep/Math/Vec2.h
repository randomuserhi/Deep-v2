...
struct DEEP_EXPORT [[nodiscard]] alignas(int64) Vec2 {
    ...
    explicit Deep_Inline Vec2(Vec3Arg in_vec);
    explicit Deep_Inline Vec2(Vec4Arg in_vec);
    ...
    [[nodiscard]] static Deep_Inline float32 Dot(Vec2Arg in_a, Vec2Arg in_b);
    [[nodiscard]] static Deep_Inline float32 Cross(Vec2Arg in_a, Vec2Arg in_b);

    [[nodiscard]] static Deep_Inline Vec2 Rotate(Vec2Arg in_a, float in_radians);
    [[nodiscard]] static Deep_Inline float32 Angle(Vec2Arg in_a, Vec2Arg in_b);

    [[nodiscard]] static Deep_Inline Vec2 Lerp(Vec2Arg in_a, Vec2Arg in_b, float32 in_t);

    //

    // Equality
    friend bool operator!=(Vec2Arg in_a, Vec2Arg in_b);
    friend bool operator==(Vec2Arg in_a, Vec2Arg in_b);

    // Add vectors
    Deep_Inline Vec2& operator+=(Vec2Arg in_other);
    friend Deep_Inline Vec2 operator+(Vec2Arg in_a, Vec2Arg in_b);

    // Sub vectors
    Deep_Inline Vec2& operator-=(Vec2Arg in_other);
    friend Deep_Inline Vec2 operator-(Vec2Arg in_a, Vec2Arg in_b);
    friend Deep_Inline Vec2 operator-(Vec2Arg in_a);
    ...
    friend Deep_Inline Vec2 operator*(Vec2Arg in_vec, float32 in_val);
    friend Deep_Inline Vec2 operator*(float32 in_val, Vec2Arg in_vec);
    ...
    friend Deep_Inline Vec2 operator*(QuatArg in_quat, Vec2Arg in_vec);
