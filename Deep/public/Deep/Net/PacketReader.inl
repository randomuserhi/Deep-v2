...
uint8 PacketReader::ReadUInt8() {
	uint8 value;
	Deep_Assert((m_head + sizeof value) <= m_tail, "Reached end of byte buffer.");
	value = *m_head;
	m_head += sizeof value;
	return value;
}
...
Vec3 PacketReader::ReadVec3() {
	Deep_Assert((m_head + sizeof(float32) * 3) <= m_tail, "Reached end of byte buffer.");
	return Vec3{ ReadFloat32(), ReadFloat32(), ReadFloat32() };
}
Vec3 PacketReader::ReadHalfVec3() {
	Deep_Assert((m_head + sizeof(float16) * 3) <= m_tail, "Reached end of byte buffer.");
