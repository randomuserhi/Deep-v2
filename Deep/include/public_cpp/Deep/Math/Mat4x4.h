...
struct [[nodiscard]] alignas(Float32x4) Mat4x4 {
	//

	Mat4x4() = default;
	Mat4x4(const Mat4x4&) = default;
	Mat4x4& operator=(const Mat4x4&) = default;
	inline Mat4x4(Arg_Float32x4 in_col0, Arg_Float32x4 in_col1, Arg_Float32x4 in_col2, Arg_Float32x4 in_col3);
	inline Mat4x4(                                                      //
	...
	inline Mat4x4(const Arg_Vec4 in_col0, const Arg_Vec4 in_col1, const Arg_Vec4 in_col2, const Arg_Vec4 in_col3);
	...
	[[nodiscard]] inline Mat4x4 transposed() const;
	...
	[[nodiscard]] inline Mat4x4 inversed() const;

	DEEP_EXPORT Mat4x4& Compose(Arg_Vec3 in_position, Arg_Quat in_rotation, Arg_Vec3 in_scale);
	...
	// Equality
	friend inline bool operator!=(Arg_Mat4x4 in_a, Arg_Mat4x4 in_b);
	friend inline bool operator==(Arg_Mat4x4 in_a, Arg_Mat4x4 in_b);

	// Mul Matrix4x4s
	friend inline Mat4x4 operator*(Arg_Mat4x4 in_a, Arg_Mat4x4 in_b);

	// Mul Matrix4x4s and float
	inline Mat4x4& operator*=(float32 in_other);
	friend inline Mat4x4 operator*(Arg_Mat4x4 in_mat, float32 in_val);
	friend inline Mat4x4 operator*(float32 in_val, Arg_Mat4x4 in_mat);
	...
	[[nodiscard]] static inline Mat4x4 s_FromQuaternion(const Quat& in_quat);
	...
	DEEP_EXPORT static const Mat4x4 k_identity;
};

static_assert(std::is_trivial_v<Mat4x4>, "Is supposed to be a trivial type!");
static_assert(std::is_standard_layout_v<Mat4x4>, "Is supposed to be standard layout!");

DEEP_NAMESPACE_END

#include "./Mat4x4.inl" // IWYU pragma: export
