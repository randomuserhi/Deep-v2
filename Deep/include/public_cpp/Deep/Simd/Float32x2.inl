...
void Float32x2::SinCos(Float32x2& out_sin, Float32x2& out_cos) {
	...
	// the sign bit)
	Int32x2 sinSign = ReinterpretAsInt() & Int32x2::s_Replicate(int32(0x80000000));
	Float32x2 _x = *this ^ sinSign.ReinterpretAsFloat();

	// x / (PI / 2) rounded to nearest int gives us the quadrant closest to x
	Int32x2 quadrant = (0.6366197723675814f * _x + Float32x2::s_Replicate(0.5f)).ToInt();
	...
	Float32x2 taylorCos = ((2.443315711809948e-5f * x2 - Float32x2::s_Replicate(1.388731625493765e-3f)) * x2
	                       + Float32x2::s_Replicate(4.166664568298827e-2f))
	                          * x2 * x2
	                      - 0.5f * x2 + Float32x2::s_Replicate(1.0f);
	// Sin(x) = x - x^3/3! + x^5/5! - x^7/7! + ... = ((-x2/7! + 1/5!) * x2 - 1/3!) * x2 * x + x
	Float32x2 taylorSin =
		((-1.9515295891e-4f * x2 + Float32x2::s_Replicate(8.3321608736e-3f)) * x2 - Float32x2::s_Replicate(1.6666654611e-1f))
			* x2 * _x
		+ _x;
	...
	Int32x2 bit2 = (quadrant << 30) & Int32x2::s_Replicate(int32(0x80000000));

	// Select which one of the results is sin and which one is cos
	Float32x2 s = Float32x2::s_Select(taylorSin, taylorCos, bit1);
	Float32x2 c = Float32x2::s_Select(taylorCos, taylorSin, bit1);
